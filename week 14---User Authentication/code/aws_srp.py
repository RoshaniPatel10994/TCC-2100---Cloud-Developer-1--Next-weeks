...
class AWSSRP(object):
    ...
    def __init__(self, username, password, pool_id, client_id, pool_region=None, client=None, client_secret=None):
    ...
    def get_auth_params(self):
        ...
        if self.client_secret is not None:
            auth_params.update({"SECRET_HASH": self.get_secret_hash(self.username, self.client_id, self.client_secret)})
        return auth_params
    ...
    def process_challenge(self, challenge_parameters):
        ...
        if self.client_secret is not None:
            response.update({"SECRET_HASH": self.get_secret_hash(self.username, self.client_id, self.client_secret)})
        return response
    ...
    def authenticate_user(self, client=None):
        ...
        response = boto_client.initiate_auth(
            AuthFlow="USER_SRP_AUTH", AuthParameters=auth_params, ClientId=self.client_id
        )
    ...
    def set_new_password_challenge(self, new_password, client=None):
        ...
        if response["ChallengeName"] == self.PASSWORD_VERIFIER_CHALLENGE:
            ...
            if tokens["ChallengeName"] == self.NEW_PASSWORD_REQUIRED_CHALLENGE:
                challenge_response = {"USERNAME": auth_params["USERNAME"], "NEW_PASSWORD": new_password}
